// Import our invoice classes and utilities
import { Invoice, Company, LineItem, utils, CONSTANTS } from './invoice.js';
...
export const app = {
    ...
    updateTotals(lineitems) {
        ...
        $("#subtotal").innerHTML = new Intl.NumberFormat('en-US', {style: 'currency', currency: 'USD' }).format(subtot);
        let tax = $("#tax").innerHTML.replace(/\D/g, '') || 0;
        $("#tax").innerHTML = "$0.00";
        let tot = parseFloat(subtot) + parseFloat(tax);
        $("#totaldue").innerHTML = new Intl.NumberFormat('en-US', {style: 'currency', currency: 'USD' }).format(tot);
...
// Make app available globally for legacy compatibility
window.app = app;
